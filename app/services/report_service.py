...
from app.agent.gpt_api_utils import call_gpt
...
class ReportService:
    ...
    async def generate_report(interview_id: str) -> ReportResponse:
        ...
        gt_agent = GroundTruthAgent(prompt_path="app/agent/prompt/ground_truth_agent.txt")
        ...
        for answer in answers:
            ...
            question = question_doc.get("question_text") or question_doc.get("question") or ""
            if not question:
                continue
            ...
            # --- GPT 호출 부분을 call_gpt()로 통일 ---
            intent_prompt = f"""
다음 질문에 대해 수험자가 제시한 답변이 질문의 의도에 적절하게 부합하는지를 평가하세요.
절대 항목을 나열하거나 장문으로 작성하지 마세요. 문장 하나로 간단명료하게 써야 합니다.

[질문]
{question}
"""
            intent = call_gpt("", intent_prompt, temperature=0.3).strip()

            evaluation_prompt = f"""
너는 지금부터 문장으로 절대 대답할 수 없어.
모든 응답은 문장이 아닌 숫자와 ,로 구성되어있어야 해.

너는 경력 있는 대학 입시 면접 컨설턴트야. 
다음 질문에 대한 답변이 어떤 카테고리에 어울리는지 판단하여 3개의 숫자 키를 선택해줘.
...
답변형태 예시: 1,2,3
"""
            evaluation_response = call_gpt("", evaluation_prompt, temperature=0.3).strip()
            ...
            # --- 모범 답변 ---
            gt_result = gt_agent.generate_ground_truth(department=major, document=answer_text, questions=[question])
            ...
            doc_processed = doc_agent.generate_document(department=major, document=answer_text).strip()
...
{doc_processed}
"""
            summary = call_gpt("너는 한국의 대학 입시 면접관이다. 반드시 문장만 간결하게 출력해야 한다.", summary_prompt, temperature=0.3).strip()
